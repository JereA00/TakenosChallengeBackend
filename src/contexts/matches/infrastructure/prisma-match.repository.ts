  ...
  implements MatchRepository
{
  ...
  async findById(id: number): Promise<MatchEntity | null> {
    ...
    if (!match.homeTeam.country || !match.awayTeam.country) {
      throw new Error(`Match ${id} has teams without country information`);
    }
  ...
  async findAll(
    ...
    const { teamId, matchDay, location, countryName } = filters;
    ...
    if (teamId) {
      ...
      where.OR = [
        { homeTeam: { country: { name: countryName } } },
        { awayTeam: { country: { name: countryName } } },
      ];
    }
